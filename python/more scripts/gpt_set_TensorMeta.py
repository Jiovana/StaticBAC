...
BIN_DIR = "gpt_tensor_binaries"
META_FILE = "gpt_tensors.meta"
os.makedirs(BIN_DIR, exist_ok=True)

# === Model ===
MODEL_NAME = "openai-community/openai-gpt"  # change here for other LMs
...
for name, param in tqdm(list(model.named_parameters()), desc="params"):
    ...
    # --- Parse layer id ---
    # GPT blocks are in transformer.h.<idx>.<submodule>
    if "transformer.h." in lname:
        try:
            layer_id = int(lname.split("transformer.h.")[1].split(".")[0])
        except Exception:
            layer_id = -1
    else:
        layer_id = -1  # embeddings, lm_head, etc.

    # --- Identify sub-block (for compression analysis) ---
    if ".attn." in lname:
        subblock = "attention"
    elif ".mlp." in lname:
        subblock = "mlp"
    ...
    # --- Identify tensor type (weight/bias/etc.) ---  
    if subblock == "layernorm" and "weight" in lname:
        layer_type = "norm_weight"
    elif "weight" in lname:
        layer_type = "weight"
    elif "bias" in lname:
        layer_type = "bias"
    ...
    if numel < 32: # do not use optimal search.. too small
        ...
        qIndex, qstep = optimal_uniform_quant(arr, 16, search_steps=50)

    # Clip to desired bitdepth BEFORE encoding (signed unless all >=0)
